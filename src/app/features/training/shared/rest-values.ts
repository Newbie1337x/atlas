 ...
 */
/** 5-second grain the whole way — matches the mental model of a stopwatch
 *  and keeps the wheel scroll uniform (no jumps at arbitrary boundaries). */
export const REST_OPTIONS: readonly number[] = (() => {
  const out: number[] = [];
  for (let s = 0; s <= 600; s += 5) out.push(s);
  return out;
})();
